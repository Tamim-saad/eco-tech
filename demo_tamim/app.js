/* ═══════════════════════════════════════════════
   NodiWatch Demo — Application Logic
   ═══════════════════════════════════════════════ */
...
function initMap() {
  ...
  L.tileLayer("https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png", {
    attribution:
      '&copy; <a href="https://carto.com/">CARTO</a> | NodiWatch Demo',
    subdomains: "abcd",
    maxZoom: 19,
  }).addTo(map);
}
...
function renderPollution() {
  ...
  pollutionData.hotspots.forEach((hs) => {
    ...
    const circle = L.circle([hs.lat, hs.lng], {
      ...
      fillOpacity: 0.12 + hs.severity / 500,
      weight: 1,
      opacity: 0.3,
    }).addTo(map);
    ...
    const innerCircle = L.circle([hs.lat, hs.lng], {
      ...
      color: TYPE_COLORS[hs.type] || COLORS.red,
      fillColor: TYPE_COLORS[hs.type] || COLORS.red,
      fillOpacity: 0.2 + hs.severity / 400,
      weight: 0,
    }).addTo(map);
...
function renderEncroachment() {
  ...
  encroachData.segments.forEach((seg) => {
    ...
    if (seg.boundary_2016 && seg.boundary_2016.length) {
      const line2016 = L.polyline(
        seg.boundary_2016.map((c) => [c[1], c[0]]),
        {
          color: COLORS.teal,
          weight: 2,
          opacity: 0.7,
    ...
    if (seg.boundary_2026 && seg.boundary_2026.length) {
      const line2026 = L.polyline(
        seg.boundary_2026.map((c) => [c[1], c[0]]),
        {
          color: COLORS.red,
          weight: 2,
          opacity: 0.8,
    ...
    marker.bindPopup(
      ...
      <div style="min-width:240px">
        ...
        <span class="popup-type" style="background:rgba(255,209,102,0.15);color:${COLORS.yellow}">ENCROACHMENT</span>
...
function showLayer(layer) {
  ...
  pollutionMarkers.forEach((m) => {
    if (showPollution) {
      m.marker.addTo(map);
      m.circles.forEach((c) => c.addTo(map));
    } else {
      map.removeLayer(m.marker);
      m.circles.forEach((c) => map.removeLayer(c));
    }
  });

  factoryMarkers.forEach((m) => {
    if (showFactories || showPollution) {
      m.marker.addTo(map);
    } else {
      map.removeLayer(m.marker);
    }
  ...
  encroachMarkers.forEach((m) => {
    if (m.marker) {
      if (showEncroach) m.marker.addTo(map);
      else map.removeLayer(m.marker);
    }
    if (m.layer) {
      if (showEncroach) m.layer.addTo(map);
  ...
  if (layer === "encroachment") {
    renderEncroachmentPanel();
  } else {
    document.getElementById("panel-rivers").classList.remove("hidden");
  }
}
  ...
  const alerts = [
    {
      type: "critical",
      icon: "🔴",
      text: "CRITICAL: Balu River dye pollution at 94% severity — Highest recorded this quarter",
      meta: "HS-008 • 2 hours ago",
      hotspot: "HS-008",
    },
    {
      type: "critical",
      icon: "🔴",
      text: "Buriganga tannery waste detected — Chromium levels 60× WHO limit",
      meta: "HS-002 • 5 hours ago",
      hotspot: "HS-002",
    },
    {
      type: "critical",
      icon: "⚠️",
      text: "Turag encroachment breach: 57% width lost at Aminbazar — Action required",
      meta: "ENC-002 • 1 day ago",
      hotspot: null,
    },
    {
      type: "warning",
      icon: "🟡",
      text: "New textile discharge detected on Turag River near dyeing cluster",
      meta: "HS-005 • 1 day ago",
      hotspot: "HS-005",
    },
    {
      type: "warning",
      icon: "🟡",
      text: "Balu River encroachment exceeded 60% threshold — Critical zone",
      meta: "ENC-003 • 2 days ago",
      hotspot: null,
    },
    {
      type: "info",
      icon: "🔵",
      text: "Shitalakshya pollution levels decreased 12% after factory inspection",
      meta: "HS-006 • 3 days ago",
      hotspot: "HS-006",
    },
    {
      type: "info",
      icon: "🛰️",
      text: "New Sentinel-2 imagery processed — All 5 rivers updated",
...
function renderRiverList() {
  ...
  container.innerHTML = riverData.features
    .map((f) => {
      const p = f.properties;
      const shrinkage =
        p.width_2016 > 0
          ? ((p.width_2016 - p.width_2026) / p.width_2016) * 100
          : 0;
      return `
...
// ═══ SIDEBAR: ENCROACHMENT PANEL ═══
function renderEncroachmentPanel() {
  const container = document.getElementById("riverList");
  if (!encroachData) return;

  container.innerHTML = encroachData.segments
    .map(
      (seg) => `
  ...
  content.innerHTML = `
    <div class="detail-section">
      ...
      <div class="detail-row">
        <span class="detail-key">Hotspot ID</span>
        <span class="detail-val">${hs.id}</span>
      </div>
      <div class="detail-row">
        <span class="detail-key">River</span>
        <span class="detail-val">${hs.river}</span>
      </div>
      <div class="detail-row">
        <span class="detail-key">Type</span>
        <span class="detail-val" style="color:${TYPE_COLORS[hs.type]}">${hs.type.toUpperCase()}</span>
      </div>
      <div class="detail-row">
        <span class="detail-key">Severity</span>
        <span class="detail-val" style="color:${sevColor}">${hs.severity}/100</span>
      </div>
      <div class="detail-row">
        <span class="detail-key">Detected</span>
        <span class="detail-val">${hs.detected}</span>
      </div>
    ...
    <div class="detail-section">
      ...
      <div class="spectral-grid">
        <div class="spectral-item">
          <div class="spectral-val" style="color:${COLORS.yellow}">${hs.spectral.ndti}</div>
          <div class="spectral-label">NDTI</div>
        </div>
        <div class="spectral-item">
          <div class="spectral-val" style="color:${COLORS.red}">${hs.spectral.redBlueRatio}</div>
          <div class="spectral-label">R/B Ratio</div>
        </div>
        <div class="spectral-item">
          <div class="spectral-val" style="color:${COLORS.purple}">${hs.spectral.thermal}°C</div>
          <div class="spectral-label">Thermal</div>
        </div>
      </div>
    </div>
...
window.showEncroachDetail = function (id) {
  ...
  content.innerHTML = `
    <div style="text-align:center;margin-bottom:24px">
      ...
      <p style="font-size:.85rem;color:${COLORS.muted}">10-Year Encroachment Analysis (Sentinel-2 satellite imagery)</p>
    </div>
...
function generateReport() {
  ...
  const criticalHotspots = pollutionData.hotspots.filter(
    (h) => h.severity >= 80,
  );
  const flaggedFactories = factoryData.factories.filter(
    (f) => f.status === "flagged",
  );
  const criticalEncroach = encroachData.segments.filter(
  ...
  content.innerHTML = `
    ...
    <div class="report-section">
      ...
      <p style="font-size:.82rem;color:${COLORS.text};line-height:1.6">
        ...
        and <strong style="color:${COLORS.yellow}">${criticalEncroach.length} critical encroachment zones</strong> across the monitored Dhaka river system.
    ...
    <div class="report-section">
      ...
      <table class="report-table">
        <thead>
          <tr><th>ID</th><th>River</th><th>Type</th><th>Severity</th><th>Detected</th></tr>
        </thead>
        <tbody>
          ${criticalHotspots
            .map(
            ...
            <tr>
              <td>${h.id}</td>
              <td>${h.river}</td>
              <td><span style="color:${TYPE_COLORS[h.type]}">${h.type.toUpperCase()}</span></td>
              <td><strong style="color:${COLORS.red}">${h.severity}/100</strong></td>
              <td>${h.detected}</td>
            </tr>
          `,
            )
    ...
    <div class="report-section">
      ...
      <table class="report-table">
        <thead>
          <tr><th>ID</th><th>Name</th><th>Type</th><th>Attribution</th><th>Violations</th></tr>
        </thead>
        <tbody>
          ${flaggedFactories
            .map(
            ...
            <tr>
              ...
              <td>${f.name}</td>
              <td>${f.type}</td>
              <td><strong style="color:${f.attribution >= 70 ? COLORS.red : COLORS.yellow}">${f.attribution}%</strong></td>
              <td>${f.violations}</td>
            </tr>
          `,
            )
    ...
    <div class="report-section">
      <h4>Encroachment Analysis — Critical Zones</h4>
      <table class="report-table">
        <thead>
          <tr><th>River</th><th>Location</th><th>2016</th><th>2026</th><th>Loss %</th><th>Affected</th></tr>
        </thead>
        <tbody>
          ${criticalEncroach
            .map(
            ...
            <tr>
              <td>${s.river}</td>
              <td>${s.location}</td>
              <td>${s.width_2016}m</td>
              <td style="color:${COLORS.red}">${s.width_2026}m</td>
              <td><strong style="color:${COLORS.red}">-${s.shrinkage_pct.toFixed(1)}%</strong></td>
              <td>~${(s.affected_population / 1000).toFixed(0)}K</td>
            </tr>
          `,
            )
            .join("")}
